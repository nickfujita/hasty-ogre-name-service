...
class Crowdsale():
    ...
    def kyc_register(self, token:Token, storage:StorageAPI, args):
        """
    ...
    def kyc_status(self, storage:StorageAPI, args):
        """
    ...
    def exchange(self, token: Token, storage:StorageAPI):
        """
    ...
    def can_exchange(self, token:Token, attachments:Attachments, storage:StorageAPI, verify_only: bool) -> bool:
        ...
        if attachments.gas_attached == 0 and attachments.neo_attached == 0:
       ...
       #     return False
        ...
        can_exchange = self.calculate_can_exchange(token, storage, amount_requested, attachments.sender_addr, verify_only)
    ...
    def calculate_can_exchange(self, token: Token, storage:StorageAPI, amount: int, address, verify_only: bool):
        """
        ...
        if amount <= token.max_exchange_limited_round:
            ...
            amount_exchanged = storage.get(r1key)
            ...
            if new_exchanged_amount <= token.max_exchange_limited_round:
                ...
                if not verify_only:
                    storage.put(r1key, new_exchanged_amount)
                return True
